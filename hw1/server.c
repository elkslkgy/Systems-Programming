...
int main(int argc, char** argv) {
    ...
    int file_fd[1025];  // fd for file that we open for reading
    char buf[512];
    int buf_len;
            ...
            if (FD_ISSET(j, &rset_copy)) {
                if (j == svr.listen_fd) {
                    ...
                    if (file_fd[conn_fd] == -1) {
                        ...
                        if (flock(file_fd[conn_fd], LOCK_NB|LOCK_SH) == 0) {
                    ...
                    do {
                        ...
                        if (file_fd[conn_fd] == -1) {
                                ...
                                // open the file here.
                            file_fd[conn_fd] = open(requestP[conn_fd].filename, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
                            if (flock(file_fd[conn_fd], LOCK_EX|LOCK_NB) == 0) {
                                write(requestP[conn_fd].conn_fd, accept_header, sizeof(accept_header));
                                fprintf(stderr, "Opening file [%s]\n", requestP[conn_fd].filename);
